...
export function Reducer (state = initialState, action) {
    switch(action.type) {
        ...
        case SET_DATA_TYPE: {
            if(action.payload === 1)
            return{...state.active_data_type, about_you: {
            ...
            if(action.payload === 2)
            return{...state.active_data_type, ads_and_businesses: {
            ...
            if(action.payload === 3)
            return{...state.active_data_type, apps_and_websites: {
            ...
            if(action.payload === 8)
            return{...state.active_data_type, groups: {
            ...
            if(action.payload === 9)
            return{...state.active_data_type, likes_and_reactions: {
            ...
            return{...state.active_data_type, location: {
                    no_data: state.user_data['no_data.txt']
                }
            }
            if(action.payload === 11)
            return{...state.active_data_type, marketplace: {
                    items_sold: state.user_data['items_sold.json'],
                }
            }
            if(action.payload === 12)
            return{...state.active_data_type, messages: {
                    archived_threads: {
            ...
            return{...state.active_data_type, photos_and_videos: {
                album: {
            ...
            return{...state.active_data_type, portal: {
                portal: {
            ...
            return{...state.active_data_type, posts: {
                ...
                your_posts: {
                    ...
                    // have to search for all of them
                }
                }
            }
            if(action.payload === 20)
            return{...state.active_data_type, profile_information: {
            ...
            return{...state.active_data_type, stories: {
                    //no data txt
                }
            }
            if(action.payload === 25)
            return{...state.active_data_type, your_places: {
                    //no data txt
                }
            }
        }
